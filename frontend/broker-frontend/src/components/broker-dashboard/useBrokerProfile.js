...
export default function useBrokerProfile({ authToken, userName, userEmail, needsProfileData }) {
  ...
  const applyProfileResponse = useCallback(
    (response, fallbackEmail = profileForm.email) => {
    ...
    async (rawEmail) => {
      ...
      try {
        ...
        setEmailVerification((prev) => ({
          ...
          requesting: false,
          info: response.data?.message || "Verification code sent to your new email.",
          error: ""
        }));
        ...
        setEmailVerification((prev) => ({
          ...prev,
          requesting: false,
          error: toShortErrorMessage(error.response?.data?.message, "Unable to send email verification code."),
          info: ""
        }));
        return false;
      }
    ...
    async (rawEmail, rawCode) => {
      ...
      try {
        const response = await api.post(
          ...
          { email: targetEmail, verificationCode },
          { headers: { Authorization: `Bearer ${authToken}` } }
        );
        ...
        setEmailVerification((prev) => ({
          ...prev,
          verifying: false,
          error: toShortErrorMessage(error.response?.data?.message, "Unable to verify email right now."),
          info: ""
        }));
      }
    },
  ...
  const handleProfileSave = useCallback(async () => {
    ...
    try {
      ...
      const pendingProfile = { ...nextProfile, email: requestedEmail, emailVerified: false };
      setProfileForm(pendingProfile);
      setProfileInitialForm(nextProfile);
      setProfileStatus({
        loading: false,
        error: "",
