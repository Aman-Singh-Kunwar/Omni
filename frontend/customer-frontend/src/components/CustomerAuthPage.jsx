...
function CustomerAuthPage({ mode = "login", onSuccess }) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 px-4 py-12">
      <div className="mx-auto w-full max-w-md rounded-2xl bg-white p-6 shadow-sm border sm:p-8">
        ...
        <h1 className="text-center text-2xl font-bold text-slate-900">Customer {mode === "login" ? "Login" : "Sign Up"}</h1>
        <p className="mt-1 text-center text-sm text-slate-500">
          {mode === "login" ? "Login to access your dashboard." : "Create a customer account to continue."}
        </p>
          ...
          {mode === "signup" && verification.pending && (
            <>
              ...
              <div className="flex flex-col items-start gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                <button type="button" onClick={resendVerificationCode} className="font-medium text-blue-600 hover:text-blue-700">
                  Send code again
                </button>
                <button
                  type="button"
        ...
        <p className="mt-4 text-center text-sm text-slate-500">
          {switchText}
          <Link to={`${switchPath}?redirect=${encodeURIComponent(redirectPath)}`} className="ml-1 font-semibold text-blue-600 hover:underline">
            {switchLabel}
          </Link>
        </p>
        {mode === "login" && (
